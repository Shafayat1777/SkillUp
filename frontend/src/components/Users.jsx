...
import { useUpdateUsersState } from "../hooks/useUpdateUserState";
...
const Users = ({ users, handleReload }) => {
  ...
  const handleChangeUserStatus = (user) => {
    if (user) {
      if (user.isBlocked) {
        updateusersstate(user.id, false);
        handleReload();
      } else {
        updateusersstate(user.id, true);
        handleReload();
      }
    }
  };
            ...
            users.map((user, i) => (
              ...
              >
                <td className="p-3 text-sm text-gray-700">
                  <div className="font-bold text-blue-500  border rounded-full px-4 py-2">
                    {i + 1}
                  </div>
                </td>
                ...
                <td className="p-3 text-sm text-gray-700">
                  {user.isBlocked ? (
                    ...
                    <div
                      onClick={() => handleChangeUserStatus(user)}
                      className="flex items-center border px-2 py-1 rounded text-normal font-semibold hover:bg-red-200 cursor-pointer"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
