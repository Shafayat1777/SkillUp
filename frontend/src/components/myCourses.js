...
const MyCourses = ({ courses, handleShowDetails, closeShowDetails }) => {
  ...
  return (
          ...
          {courses.map((course) => (
            <tr key={course.id} className={i % 2 !== 0 ? "bg-white" : "bg-gray-50"}>
              ...
              <td className="p-3 text-sm text-gray-700 whitespace-nowrap">
                <h3 onClick={()=>{handleShowDetails(course.id)}} className="font-bold text-blue-500 hover:underline rounded-full px-4 py-2 hover:bg-blue-200 cursor-pointer">
                  {course.title}
                </h3>
              </td>
              <td className="p-3 text-sm text-gray-700 whitespace-nowrap">
    ...
    {success && (
        <div className="mt-4 w-full border rounded border-green-400 text-center text-green-400 bg-green-100 tracking-wider">
          {success}
        </div>
      )}
    </div>
